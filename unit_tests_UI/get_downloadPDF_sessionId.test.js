...
describe('GET /downloadPDF/{sessionId} API Tests', () => {
    // this.timeout(50000);
    // eslint-disable-next-line no-undef
    it('Scenario 1: Should download pdf for a valid sessionId ', (done) => {
        const validSessionId = ''; // Replace with a valid sessionId

        chai.request(api_url)
            .get(`/downloadPDF/${validSessionId}`)
            .end((err, res) => {
    ...
    it('Scenario 2: Should return an error for an invalid sessionId', (done) => {
        ...
        chai.request(api_url)
            .get(`/downloadPDF/${invalidSessionId}`)
            .end((err, res) => {
                console.log('Response Body:', res.body);
                expect(res).to.have.status(404); // or 404, depending on your expected behavior
    ...
    it('Scenario 3: Should return an error for an empty sessionId', (done) => {
        ...
        chai.request(api_url)
            .get(`/downloadPDF/${emptySessionId}`)
            .end((err, res) => {
                console.log('Response Body:', res.body);
